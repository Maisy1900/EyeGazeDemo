...
public class OVRSkeleton : MonoBehaviour
{
    ...
    protected void UpdateSkeleton()
    {
        ...
        if (_updateRootPose)
        {
            Vector3 rootPos = data.RootPose.Position.FromFlippedZVector3f();
            Quaternion rootRot = data.RootPose.Orientation.FromFlippedZQuatf();

            if (_rootOverride != null)
            {
                // Compute world transform from tracking pose
                Matrix4x4 trackingMatrix = Matrix4x4.TRS(rootPos, rootRot, Vector3.one);

                // Apply the override transform as a parent
                Matrix4x4 offsetMatrix = _rootOverride.localToWorldMatrix;

                // Final pose is offset * tracking pose
                Matrix4x4 finalPose = offsetMatrix * trackingMatrix;

                transform.position = finalPose.GetColumn(3); // Translation
                transform.rotation = finalPose.rotation;
            }
            else
            {
